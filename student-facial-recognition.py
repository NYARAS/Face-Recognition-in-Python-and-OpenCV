...
os.environ["GOOGLE_APPLICATION_CREDENTIALS"] = "C:\\Users\\Calvinerea De Linux\\Downloads\\pythonapp-65963b11f84c.json"
...
class Profile():
    ...
    def __init__(self, master, idNum):
        ...
        if userID == self.idNum:
            ...
            tempName = self.name
            self.school = userAtts.get('School')
            self.number = userAtts.get('Number')
            self.course = userAtts.get('Course')
        print(self.idNum)
        self.getOnlineImages()
        img = ImageTk.PhotoImage(Image.open('Pictures/' + tempName + '/image_3.jpg'))
        label = tk.Label(self.window, image=img)
        label.pack()
        label2 = tk.Label(self.window,text="Student Name:" +" "+tempName,font="Times 16 bold")
        label2.pack()
        label3 = tk.Label(self.window, text="Adm.No:" + "  " + self.number, font="Times 16 bold")
        label3.pack()
        label4 = tk.Label(self.window, text="School:" + "  " + self.school,font="Times 16 bold")
        label4.pack()
        label5 = tk.Label(self.window, text="Course:" + "  " + self.course,font="Times 16 bold")
        label5.pack()
...
class FaceRecognize():
    ...
    def __init__(self, master):
        numValues = []
        recognizer.read('trainerSet.yml')
...
class Fifth():
    ...
    def __init__(self, master, idNum):
        ...
        recognizer.write('trainerSet.yml')
        cv2.destroyAllWindows()
    ...
    def uploadRecognizer(self):
        ...
        imageBlob.upload_from_filename("trainerSet.yml")
...
class Second():
    ...
    def __init__(self, master):
        ...
        label2 = tk.Label(self.window, text="School")
        self.entry2 = tk.Entry(self.window)
        label2.grid(row=1)
        self.entry2.grid(row=1, column=1)

        label3 = tk.Label(self.window, text="Course")
    ...
    def submitData(self):
        ...
        result = firebase.put('/', self.idNum,
                              {'Name': self.entry1.get(), 'School': self.entry2.get(), 'Course': self.entry3.get(),
                               'ID': self.idNum, 'Number': self.entry4.get()})
        self.changeWindow()
...
class Home():
    ...
    def __init__(self, master):
        ...
        root.title("Facial Recognition Homepage")
        ...
        button2.pack()
        button3 = tk.Button(root, text="Train Dataset", width=15, height=3, command=self.train)
        button3.pack()
    ...
    def train(self):
        ...
        train = Fifth(root, self.idNum)


if __name__ == '__main__':
    root = tk.Tk()
    firebase = firebase.FirebaseApplication('https://pythonapp-9b846.firebaseio.com/')
    recognizer = cv2.face.LBPHFaceRecognizer_create()
    client = storage.Client()
    bucket = client.get_bucket('pythonapp-9b846.appspot.com')
